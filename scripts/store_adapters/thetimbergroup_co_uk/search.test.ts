...
function load_recording(): Recording {
  ...
  if (
    ...
    throw new Error("recordings.json: missing queries array");
  }
...
function title_overlap(
  ...
  const actual_titles: string[] = [];
  for (const p of actual) {
    const t = p.title;
    if (typeof t === "string") actual_titles.push(t.toLowerCase());
  }
  const actual_urls: string[] = [];
  for (const p of actual) {
    const u = p.url;
    if (typeof u === "string") actual_urls.push(u);
  }
...
describe("thetimbergroup.co.uk search adapter", () => {
